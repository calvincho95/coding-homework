...
// - 답안
    ...
    function getDigit(uint) public pure returns {
...
// 고객의 정보를 넣고 변화시키는 함수를 구현하세요.
    ...
    maaping(uint => Customer) customers;

    function addCustomer(uint _number, string _name, bytes _hash) public {
        bytes(_hash) = keccak256(abi.encodePacked(_number, _name));   


    function getCustomer(uint _number) public view returns(customers) {
        return customers;
    }
}
